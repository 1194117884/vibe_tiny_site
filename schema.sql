...
CREATE TABLE IF NOT EXISTS plan_entitlements (
  ...
  source_type TEXT NOT NULL CHECK (source_type IN ('activation_code', 'simulated_purchase', 'admin_grant', 'stripe_invoice')), source_ref TEXT,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_entitlements_stripe_invoice ON plan_entitlements(source_ref) WHERE source_type = 'stripe_invoice';
CREATE TABLE IF NOT EXISTS stripe_customers (
  user_id TEXT PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  stripe_customer_id TEXT NOT NULL UNIQUE, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL
);
CREATE TABLE IF NOT EXISTS stripe_subscriptions (
  stripe_subscription_id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  plan_id TEXT NOT NULL REFERENCES plans(id), stripe_customer_id TEXT NOT NULL,
  status TEXT NOT NULL, cancel_at_period_end INTEGER NOT NULL DEFAULT 0,
  current_period_end INTEGER, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_stripe_subscriptions_user ON stripe_subscriptions(user_id, status, updated_at DESC);
CREATE TABLE IF NOT EXISTS stripe_payments (
  id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  plan_id TEXT NOT NULL REFERENCES plans(id), stripe_invoice_id TEXT NOT NULL UNIQUE,
  stripe_payment_intent_id TEXT UNIQUE, stripe_customer_id TEXT, stripe_subscription_id TEXT,
  amount_total INTEGER NOT NULL DEFAULT 0, amount_refunded INTEGER NOT NULL DEFAULT 0,
  currency TEXT NOT NULL, status TEXT NOT NULL, failure_message TEXT,
  entitlement_id TEXT REFERENCES plan_entitlements(id), created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_stripe_payments_user ON stripe_payments(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_stripe_payments_subscription ON stripe_payments(stripe_subscription_id, created_at DESC);
CREATE TABLE IF NOT EXISTS stripe_events (
  stripe_event_id TEXT PRIMARY KEY, event_type TEXT NOT NULL,
  status TEXT NOT NULL, error_message TEXT, created_at INTEGER NOT NULL, processed_at INTEGER
);
